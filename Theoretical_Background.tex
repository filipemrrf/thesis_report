...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\section{Convergence of a Numerical Scheme}
%\label{section:convergence}

%Unfortunately, numerical relativity doesn't provide us with exact solutions to our equations. What it does provide us are approximated solutions, which raise the question "how good are these approximations?". If one has the exact solution to the problem in question, the answer becomes trivial. However, often we don't have an exact solution to compare our results with. In those cases, we need to compare our numerical solutions to other numerical solutions.

%Numerical schemes are designed such that the numerical solutions converge towards the exact solution as we increase resolution. This property allows us to check how accurate our solutions are by comparing different resolutions among themselves. If the numerical solutions are getting closer together as we increase resolution, it is a strong sign that they are converging towards the exact solution of our problem \cite{Introduction_to_3+1_numerical_relativity,Numerical_Relativity_Solving_Einsteins_Equations_on_the_Computer}. In this work, however, we take a slightly different approach. Instead of comparing the solutions themselves, we will be comparing the value of the constraints of our equations (which will be defined in section \ref{section:wave_equation}). Similarly to the distance between the numerical solutions, it is expected that the constraint violations (which should be due to truncation error) decrease with the increase in resolution. Thus, we will monitor the value of the $L^2$ norm of our constraints and use it to validate our results.
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.6\textwidth]{Figures/Definitions_of_the_3+1_quantities.png}
...
\end{align}
%
where $\overset{!}{=}$ denotes that we may allow for small violations of these constraints due to numerical errors. The second constraint is of particular importance as we will see later in section \ref{section:hyp_wave}. Due to this importance, we will denote it as $B_i$.
...
In order to keep constraint violations from growing exponentially, it may be useful to add multiples of the constraint $B_i$~to the evolution equations of our system since doing so does not alter the physical behavior of the system, but it improves numerical stability. Doing so, we obtain the final form of our system as
%
\begin{align}
    \begin{cases}
...
It is known that, for small initial data, the dispersive nature of the wave operator dominates, leading to the global existence of solutions which decay as $t^{-2}$ near timelike infinity and as $t^{-1}$ along null infinity \cite{Universality_of_global_dynamics_for_the_cubic_wave_equation}.. However, for large initial data, the focusing effect of the cubic nonlinearity can lead to the blowup of solutions with a rate of $\sqrt{2}/(T-t)$ at the origin \cite{Universality_of_global_dynamics_for_the_cubic_wave_equation}.
...
\end{align}
%
This family of solutions also has the interesting property that, when dividing the space of solutions into three regions based on their asymptotic behavior, we have a direct correspondence with the value of $b$: decaying solutions have $b > 0$, blowup solutions have $b < 0$ and critical solutions have $b = 0$. It is also conjectured that for any generic globally regular solution $\psi(T, R)$ there exist parameters $(a,b) \in \mathbb{R} \times \mathbb{R}^{+} $ and $\kappa = \pm 1$ such that $t^4 \left(\psi(T, R) - \kappa \psi_{(a, b)}(T, R) \right)$ is bounded for all finite $r$ and $t \to \infty$ \cite{Universality_of_global_dynamics_for_the_cubic_wave_equation}.

To study this equation numerically, we can set the source term $S$~in equation \eqref{eq:wave_equation_constraint_violation} to be $S = \Psi^3 = \Omega^3 \psi^3$~, obtaining 
%
\begin{align}
    \begin{cases}
        \partial_t \tilde{\psi} = - \tilde{\alpha} \tilde{\Pi} + \beta^i \tilde{\phi}_i + (1 + \gamma_1) \beta^i (\tilde{D}_i \tilde{\psi} - \tilde{\phi}_i) \\
